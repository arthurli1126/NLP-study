...
def class32(X_train, X_test, y_train, y_test, iBest):
   ...
   '''
    ...
    X_1k, y_1k = sampler(X_train, y_train, 1000)
    X_5k, y_5k = sampler(X_train, y_train, 5000)
    X_10k, y_10k = sampler(X_train, y_train, 10000)
    X_15k, y_15k = sampler(X_train, y_train, 15000)
    X_20k, y_20k = sampler(X_train, y_train, 20000)
...
def class33(X_train, X_test, y_train, y_test, i, X_1k, y_1k):
    ...
    with open('a1_3.3.csv', 'w', newline='') as csvfile:
        ...
        for i in k:
            ...
            selector_1k = SelectKBest(f_classif, i)
            X_new = selector.fit_transform(X_train, y_train)
            selector_1k.fit_transform(X_1k, y_1k)
            indices = selector.pvalues_.argsort()[:i]
            indices_1k = selector_1k.pvalues_.argsort()[:i]
            if i == 5: print("top five features {}".format(indices))
            print("common indices{}".format([p for p in indices if p in indices_1k]))
            file_writer.writerow([i] + selector.pvalues_[indices].tolist())
...
if __name__ == "__main__":
    ...
    t_1 = time.time()
    print("class31 time {} class32".format(t_1 - t_0))
    X_1k, y_1k = class32(X_train, X_test, y_train, y_test, iBest)
    t_2 = time.time()
    print("class32 time {} class33".format(t_2 - t_1))
    class33(X_train, X_test, y_train, y_test, iBest, X_1k, y_1k)
    print("class34:{}".format(iBest))
    class34(args.input, iBest)
    print("total_Time: {}".format(time.time() - t_0))
